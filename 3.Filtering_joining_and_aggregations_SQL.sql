...
with vw_salary as(
	SELECT department, salary AS is_higher_than_dept_avg_salary FROM staff s 
	where salary > (SELECT ROUND(AVG(salary),2)
					 FROM staff)
	)
--select * from vw_salary

SELECT department,sum(is_higher_than_dept_avg_salary) as sum_total,COUNT(2) AS total_employees
FROM vw_salary 
group by department
...
CREATE OR Alter VIEW vw_staff_div_reg_country AS
