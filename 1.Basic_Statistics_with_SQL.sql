...
/* Data Interpretation: It seems like the average between male and female group is pretty close, with slightly higher average salary for Female group*/
...
SELECT SUM(salary) as total_yearly_salary
FROM staff;
...
SELECT 
	...
	MAX(salary) AS Max_Salary, 
	AVG(salary) AS Average_Salary,
	ROUND(VARP(salary),3) AS Variance_Salary,
	Round(STDEVP(salary),3) AS StandardDev_Salary,
	COUNT(*) AS total_employees
FROM staff
GROUP BY department
ORDER BY 4 DESC;
...
SELECT 
	...
	ROUND(VARP(salary),2) AS Variance_Salary,
	ROUND(STDEVP(salary),2) AS StandardDev_Salary,
	COUNT(*) AS total_employees
FROM staff
GROUP BY department
ORDER BY 6 DESC;
...
for Health deparment may be a little bit more stretch than Outdoors deparment which has more high earners.
